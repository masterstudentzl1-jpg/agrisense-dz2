...
.alert-meta{font-size:0.7rem;color:#9ca3af;display:flex;gap:10px;flex-wrap:wrap}
.alert-badge{font-size:0.72rem;font-weight:700;padding:3px 10px;border-radius:50px;white-space:nowrap;flex-shrink:0}
...
.field-card-icon-wrap{width:44px;height:44px;border-radius:12px;background:#f0fdf4;display:flex;align-items:center;justify-content:center;font-size:22px}
...
.field-reading-icon{font-size:16px;margin-bottom:3px}
...
function BarChart({ groups, labels, colors, height=200, yMax }) {
  ...
  const gap=bw*0.2,groupW=cw/labels.length
  ...
  return(
    <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg" style={{height}}>
      ...
      {labels.map((l,gi)=>{const gx=PL+gi*groupW+groupW*0.2;return<g key={gi}>{groups.map((series,si)=>{const bh=(series[gi]/hi)*ch,x=gx+si*(bw+gap),y=PT+ch-bh;return<rect key={si} x={x} y={y} width={bw} height={bh} fill={colors[si]} rx="4"/>})}<text x={PL+gi*groupW+groupW/2} y={H-5} fontSize="10" fill="#9ca3af" textAnchor="middle">{l}</text></g>})}
    </svg>
  )
}
          ...
          <div className="db-content">
            ...
            {tab==='overview'&&(<>
              ...
              <div className="chart-card">
                <div className="chart-head"><div className="chart-head-left"><h3>Soil Moisture — 7 Days</h3><p>By field region (%)</p></div><span className="chart-icon">💧</span></div>
                  ...
                  <div key={i} className="field-card">
                    ...
                    <div className="field-card-body">
                      ...
                      <div className="field-readings">
                        <div className="field-reading"><div className="field-reading-icon">💧</div><div className={`field-reading-val ${f.moisture.c}`}>{f.moisture.v}</div><div className="field-reading-key">Moisture</div></div>
                        <div className="field-reading"><div className="field-reading-icon">🌡️</div><div className={`field-reading-val ${f.temp.c}`}>{f.temp.v}</div><div className="field-reading-key">Temp.</div></div>
                        <div className="field-reading"><div className="field-reading-icon">📡</div><div className={`field-reading-val ${f.sensors.c}`}>{f.sensors.v}</div><div className="field-reading-key">Sensors</div></div>
                      </div>
            ...
            {tab==='analytics'&&(<>
              ...
              <div className="analytics-kpi-grid">
                {[{icon:'💧',name:'Avg. Soil Moisture',val:'41%',change:'-4% vs last period',pos:false},{icon:'🌡️',name:'Peak Temperature',val:'35.2°C',change:'+2.1°C vs last period',pos:false},{icon:'↗',name:'Yield Improvement',val:'+18%',change:'vs same period 2024',pos:true},{icon:'🌧️',name:'Total Rainfall',val:'12.4 mm',change:'-8 mm vs last period',pos:false}].map((k,i)=>(
